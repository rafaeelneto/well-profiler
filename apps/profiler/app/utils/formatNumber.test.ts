...
describe('formatNumber', () => {
  ...
  describe('locale decimal separators', () => {
    it('formats with period separator in en-US', () => {
      expect(formatNumber(3.28, { fractionDigits: 2, locale: 'en-US' })).toBe('3.28')
    })

    it('formats with comma separator in pt-BR', () => {
      expect(formatNumber(3.28, { fractionDigits: 2, locale: 'pt-BR' })).toBe('3,28')
    })
  })

  describe('fractionDigits', () => {
    it('pads with zeros (fixed precision)', () => {
      expect(formatNumber(1.5, { fractionDigits: 2, locale: 'en-US' })).toBe('1.50')
    })

    it('rounds to given digits', () => {
      expect(formatNumber(1.555, { fractionDigits: 2, locale: 'en-US' })).toBe('1.56')
    })

    it('fractionDigits: 0 produces integer', () => {
      expect(formatNumber(42.9, { fractionDigits: 0, locale: 'en-US' })).toBe('43')
    })
  })

  describe('maximumFractionDigits', () => {
    it('strips trailing zeros', () => {
      expect(formatNumber(1.5, { maximumFractionDigits: 4, locale: 'en-US' })).toBe('1.5')
    })

    it('does not exceed max digits', () => {
      expect(formatNumber(1.23456, { maximumFractionDigits: 2, locale: 'en-US' })).toBe('1.23')
    })
  })
  ...
  describe('suffix', () => {
    ...
    it('appends m³ suffix', () => {
      expect(formatNumber(2.5, { fractionDigits: 2, suffix: 'm³', locale: 'en-US' })).toBe('2.50 m³')
    })
  ...
  describe('locale override in options', () => {
    it('uses provided locale over default', () => {
      expect(formatNumber(1.5, { fractionDigits: 2, locale: 'pt-BR' })).toBe('1,50')
    })
  ...
  describe('thousands separator', () => {
    it('formats large numbers with en-US grouping', () => {
      expect(formatNumber(1000000, { fractionDigits: 0, locale: 'en-US' })).toBe('1,000,000')
    })

    it('formats large numbers with pt-BR grouping', () => {
      expect(formatNumber(1000000, { fractionDigits: 0, locale: 'pt-BR' })).toBe('1.000.000')
    })
  })
})
