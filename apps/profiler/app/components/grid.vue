<template>
  <div class="hr-demo">
    <div class="hr-toolbar">
      <span class="text-sm font-medium opacity-70">Data Source:</span>
      <select
        class="hr-select"
        v-model="currentSize"
        @change="onSizeChange"
      ...
      <div v-if="loading" class="text-sm opacity-50 animate-pulse ml-2">
        Loading data...
      </div>
    </div>
...
import { getHRData, HR_OPTIONS } from '../sys-data/hr.data';
import {
  getBaseHRColumns,
  getExtraHRColumns,
  HR_COLOR_BY_AGE,
} from '../sys-data/hr.columns';
import { currentThemeVue } from '../composables/useRandomData';
import {
  createHRColorSelectColumnType,
  renderHrColorPill,
} from './hr-color-select';
import './hr-demo.css';
...
const gridTheme = computed(() =>
  props.isDark === true || pageIsDark.value ? 'darkCompact' : 'compact',
);
...
const columns = computed(() => {
  const dropdownSource: string[] = Array.from(
    new Set(rows.value.map(r => r.company)),
  ).filter(Boolean);
  ...
  eyesCol.cellTemplate = (h, props) => renderHrColorPill(h, props.value);

  return [...baseCols, ...getExtraHRColumns(100)];
});

async function loadData() {
  loading.value = true;
  try {
    rows.value = await getHRData(currentSize.value);
  } finally {
    loading.value = false;
  }
}
