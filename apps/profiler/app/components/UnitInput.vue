...
function toCanonical(v: number): number {
  if (props.unitType === 'length') return uiStore.lengthUnit === 'ft' ? feetToMeters(v) : v
  return uiStore.diameterUnit === 'inches' ? inchesToMm(v) : v
}

const displayValue = computed(() => (props.modelValue != null ? toDisplay(props.modelValue) : null))
...
const displayStep = computed(() => (props.step != null ? toDisplay(props.step) : undefined))
</script>
...
<template>
  <WellInputNumber
    ...
    @update:model-value="(v) => emit('update:modelValue', v != null ? toCanonical(v) : null)"
  />
</template>
